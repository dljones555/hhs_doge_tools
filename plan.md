...
## Backlog
- [ ] **Add NPI entity type awareness to detector** — Currently the capacity model treats all WORKER_NPIs as individuals, but org NPIs (Type 2) self-listing as servicing provider can have multiple staff underneath. The "impossible hours" flag is only valid for Type 1 (individual) NPIs. Options: (a) NPPES batch lookup on flagged entities post-detection, (b) add entity_type column to output, (c) separate scoring thresholds for org vs individual NPIs.
